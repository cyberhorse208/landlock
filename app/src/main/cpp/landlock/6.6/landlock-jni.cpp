...
static bool populate_ruleset_new(char * env_path_name, const int ruleset_fd,
                                const __u64 allowed_access)
{
    ...
    if (!env_path_name) {
        ...
        LOGE("Missing environment variable");
        return false;
    }
...
Java_com_vivo_kmirrors_security_Landlock_SetFileRules(JNIEnv *env, jobject thiz,
    ...
    if (ruleset_fd < 0) {
        LOGE("Failed to create a ruleset");
        return -1;
    }
...
Java_com_vivo_kmirrors_security_Landlock_testString(JNIEnv *env, jobject thiz,
    ...
    // 检查结果是否为 NULL，避免程序崩溃
    if (roPaths == NULL || rwPaths == NULL) {
        // 处理转换错误: 返回空字符串或相应的错误信息
        return env->NewStringUTF("");
    }

    // 计算拼接后字符串的总长度
    size_t totalLength = strlen(roPaths) + strlen(rwPaths) + 1; // +1 for null terminator
