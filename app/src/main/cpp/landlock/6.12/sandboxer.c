...
static const char help[] =
	...
	"%1$s -r /data/input/hello.txt -w /data/output/result.txt\n"
	"\n"
	"This sandboxer6.12.so can use Landlock features up to ABI version "
...
int main(const int argc, char *const argv[], char *const *const envp)
{
	...
	case 1:
		 ...
		 * Landlock with ABI 1.
		 *
		 * If only ABI 1 is available, this sandboxer6.12.so knowingly forbids
		 * refer operations.
		 *
	...
	default:
		fprintf(stderr,
			"Hint: You should update this sandboxer6.12.so "
			"to leverage Landlock features "
			"provided by ABI version %d (instead of %d).\n",
